...
public class ProjectService {
    ...
    public ProjectService(ProjectRepository projects, ConfigRepository configs, UserRepository users,
                          JobRepository jobs, SupportRepository support) {
    ...
    public CreateProjectResponse createGithub(User user, CreateGithubProjectRequest request) {
        ...
        support.audit(user.userId(), project.projectId(), null, "PROJECT_CREATED", "PROJECT", project.projectId().toString());
        return new CreateProjectResponse(project.projectId(), project.projectName(), project.sourceType(), project.status(), project.createdAt());
    }

    public CreateProjectResponse createZip(User user, ZipProjectMetadata metadata) {
        Project project = projects.create(user.userId(), metadata.projectName(), metadata.description(), SourceType.ZIP,
                null, "projects/" + UUID.randomUUID(), metadata.trainingEntrypoint());
        configs.createDefault(project.projectId(), project.trainingEntrypoint());
        support.audit(user.userId(), project.projectId(), null, "PROJECT_CREATED", "PROJECT", project.projectId().toString());
        return new CreateProjectResponse(project.projectId(), project.projectName(), project.sourceType(), project.status(), project.createdAt());
    }
    ...
    public ProjectDetail detail(Project project) {
        ...
        ProjectSummary summary = summary(project);
        return new ProjectDetail(project.projectId(), project.projectName(), project.description(), project.sourceType(),
                summary.latestJobStatus(), summary.lastTrainingTime(), summary.lastTrainingOwner(), project.repositoryUrl(),
    ...
    public void delete(User user, Project project) {
        ...
        support.audit(user.userId(), project.projectId(), null, "PROJECT_DELETED", "PROJECT", project.projectId().toString());
    }
