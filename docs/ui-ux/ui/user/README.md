...
- **Framer Motion Direction**: In implementation, use `framer-motion` to bring these screens to life without compromising data clarity. Apply soft page fades, staggered card reveals, dialog scale/fade transitions, gentle `RUNNING` status pulses, notification enter/exit transitions, and slow ambient drift for the light-mode aurora background. Disable movement or reduce it to opacity-only transitions when `prefers-reduced-motion` is active.
